...
namespace engine::resources {
    ...
    void Mesh::draw_instanced(const Shader *shader, const int size) {
        std::unordered_map<std::string_view, uint32_t> counts;
        std::string uniform_name;
        uniform_name.reserve(32);
        for (int i = 0; i < m_textures.size(); i++) {
    ...
    void Mesh::initialization_mesh_instances(const std::vector<glm::mat4> &instances) {
        GLuint buf = 0;
        CHECKED_GL_CALL(glGenBuffers, 1, &buf);
        CHECKED_GL_CALL(glBindVertexArray,m_vao);
        CHECKED_GL_CALL(glBindBuffer,GL_ARRAY_BUFFER, buf);
        CHECKED_GL_CALL(glBufferData,GL_ARRAY_BUFFER, instances.size() * sizeof(glm::mat4),
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            CHECKED_GL_CALL(glVertexAttribPointer,index, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
                                        (const void*)(sizeof(glm::vec4) * i));
            CHECKED_GL_CALL(glVertexAttribDivisor, index, 1);
        }
    }
